...
namespace vaatv::internal {
    ...
    public:
        ...
        using container_type = std::vector<device_t>;
        using ref_container_type = std::vector<std::reference_wrapper<device_t>>;
    ...
    protected:
        ...
        void login_devices(const auto &error_callback) noexcept requires concepts::user_device<device_t> {
            for (auto &device : m_devices) {
                if (!device.login()) { error_callback(device); }
            }
        }

        void logout_devices() noexcept requires concepts::user_device<device_t> {
